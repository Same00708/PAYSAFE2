-- PaySafe : notifications.user_id doit pointer vers paysafe_users (pas users Supabase)
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'paysafe_users'
  ) THEN
...
-- transaction_id → transactions (si la table existe)
